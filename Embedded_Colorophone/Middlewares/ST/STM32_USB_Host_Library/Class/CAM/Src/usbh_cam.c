...
void USBH_CAM_InitProbeSet(ProbeData_Typedef* probeCtrl)
{
	...
	//Init for 320x240 30fps Uncompressed (YAV2) Video stream
...
USBH_StatusTypeDef USBH_CAM_GetParamRequests (USBH_HandleTypeDef* phost, CAM_HandleTypeDef* CAM_Handle)
{
 	...
 	switch(CAM_Handle->getParam_state)
	{
			case CAM_REQ_GET_INIT:
				CAM_Handle->getParam_state = CAM_REQ_BRIGHTNESS_SET_CUR;
#if (USBH_USE_OS == 1)
    osMessagePut ( phost->os_event, USBH_STATE_CHANGED_EVENT, 0);
#endif
				break;
			...
			//Brightness CUR
			case CAM_REQ_BRIGHTNESS_SET_CUR:
				CAM_Handle->param = 0xFF;
				testStatus = USBH_CSRequest(phost, USB_REQ_SET_CUR, PU_BRIGHTNESS_CONTROL, PROCESSING_UNIT);
				if(testStatus == USBH_OK)
				{
				...
				{
					CAM_Handle->getParam_state = CAM_REQ_TEST_ERROR;
				}
			break;
...
USBH_StatusTypeDef USBH_CAM_ConfigDevice(USBH_HandleTypeDef* phost, CAM_HandleTypeDef* CAM_Handle)
{
	...
	switch(CAM_Handle->cfg_state)
	{
		case CAM_REQ_CFG_INIT:
			CAM_Handle->cfg_state = CAM_REQ_GET_PARAMS;
#if (USBH_USE_OS == 1)
    osMessagePut ( phost->os_event, USBH_STATE_CHANGED_EVENT, 0);
#endif
    		LCD_UsrLog("cfg INIt");
